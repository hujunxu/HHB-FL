...
# Calculate average attack success rates
average_results = {attack: {method: np.mean(results[attack][method]) for method in methods} for attack in attack_types}

# Save results to CSV file
df = pd.DataFrame.from_dict(average_results, orient='index')
df.to_csv('E:\\桌面\\攻击\\attack_success_rates.csv', index=True)
...
for attack in attack_types:
    ...
    for method in methods:
        print(f"{method}: {average_results[attack][method] * 100:.2f}%")
